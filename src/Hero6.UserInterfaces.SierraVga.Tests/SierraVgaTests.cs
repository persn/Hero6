...
namespace LateStartStudio.Hero6.Engine.UserInterfaces.SierraVga
{
    ...
    public class SierraVgaTests : UserInterfaceTestBase<SierraVgaModule>
    {
        ...
        public void CursorResetToWalkOnMiddleMouseButtonPress()
        {
            ...
            var controller = Substitute.For<CursorController>(expected, Services.Container, Services.Mouse, Services.Assets, Services.Renderer);
        ...
        {
            base.PreInitialize();
            var statusBar = Substitute.For<WindowController>(Substitute.For<IWindowModule>(), Services.Container, Services.GameSettings, Services.ControllerRepository, Services.Renderer, Services.GraphicsDevice);
            Controller.GetWindow<StatusBar>().Returns(statusBar);
            var walk = Substitute.For<CursorController>(Substitute.For<ICursorModule>(), Services.Container, Services.Mouse, Services.Assets, Services.Renderer);
            ...
            where TAfter : class, ICursorModule
        {
            ...
            var after = Substitute.For<CursorController>(Substitute.For<ICursorModule>(), Services.Container, Services.Mouse, Services.Assets, Services.Renderer);
